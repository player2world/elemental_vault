...
describe("elemental_vault", () => {
  ...
  try {
    creator = getKpFromSecretKey(process.env.CREATOR_KEY);
    authority = getKpFromSecretKey(process.env.AUTHORITY_KEY);
  } catch (error) {
  ...
  it("Authority Close Escrow", async () => {
    ...
    accounts.vaultAta = getAssociatedTokenAddressSync(
      selectedVault.account.baseMint,
      selectedVault.publicKey,
      true
    );
    console.log("TEST 1");
    console.log("SourceATa", accounts.vaultAta);
    console.log("TEST 2");
    console.log("vaultATa", accounts.authorityMintAta);
    try {
      await program.methods
        .closeVault(selectedVault.account.vaultCount, authority.publicKey)
        .accounts({
